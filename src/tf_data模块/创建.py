...
data_arr = [[1,2],[3,4],[5,6]] # <TensorSliceDataset shapes: (2,), types: tf.int32>
data_dcit = {'A':[1],'B':[2],'C':[3]}  # <TensorSliceDataset shapes: {A: (), B: (), C: ()}, types: {A: tf.int32, B: tf.int32, C: tf.int32}>

data_set = tf.data.Dataset.from_tensor_slices(data_arr)
...
data_set = data_set.shuffle(buffer_size=7)  # 乱序
data_set = data_set.repeat(count=3)  # 且重复干 3 次这种乱序的事。。。目的，防止模型记住了数据的顺序
data_set = data_set.batch(batch_size=3)  # 批次
