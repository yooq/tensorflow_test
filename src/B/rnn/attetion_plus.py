...
class Encoder(keras.Model):
    def __init__(self,vocab_size,embedding_dim,hidden_units):
        ...
        # encoder
        self.encoder = keras.layers.LSTM(hidden_units,return_sequences=True,return_state=True)
...
class Decoder(keras.Model):
    ...
    def call(self,enc_output,dec_inputs,state_inputs):
        ...
        dec_attention = self.attention([dec_output,enc_output])  #两个大hidden
...
def seq2seq(maxlen,embedding_dim,hidden_units,vocab_size):
    ...
    attention_output, dec_state_h, dec_state_c = decoder(enc_outputs, dec_input, dec_states_inputs)
...
def decoder(model,encoder):
    ...
    dec_output,out_dec_h,out_dec_c = decoder(enc_output,dec_input,dec_states_input)
